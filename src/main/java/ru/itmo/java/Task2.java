...
public class Task2 {
    ...
    boolean numberInRange(Integer number, Integer leftBound, Integer rightBound, Boolean inclusively) {
        if (number == null || leftBound == null || rightBound == null || inclusively == null)
            return false;
        if (inclusively)
            return number.intValue() >= leftBound && number.intValue() <= rightBound;
        else
            return number.intValue() > leftBound && number.intValue() < rightBound;
    }
    ...
    boolean atLeastOneIsDigit(char c1, char c2, char c3) {
        int leftBound = 48;
        int rightBound = 57;
        return c1 >= leftBound && c1 <= rightBound ||
                c2 >= leftBound && c2 <= rightBound ||
                c3 >= leftBound && c3 <= rightBound;
    }
    ...
    int schoolDesks(int num1, int num2, int num3) {
        return (int)Math.ceil(num1 / 2.0) + (int)Math.ceil(num2 / 2.0) + (int)Math.ceil(num3 / 2.0);
    }
